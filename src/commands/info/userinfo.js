...
module.exports = class UserInfoCommand extends Command {
  ...
  async run(message, args) {
    ...
    // const activities = [];
    // let customStatus;
    // for (const activity of member.presence ? member.presence.activities : []) {
    ...
    // Trim roles
    // let roles = message.client.utils
    //   .trimArray(member.roles.cache.filter((r) => !r.name.startsWith("#")))
    //   .filter((r) => r != message.guild.roles.everyone);
    // roles = roles.sort((a, b) => b.position - a.position);
    // roles = roles.values().map((v) => v.toString());
