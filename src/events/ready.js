module.exports = async (client) => {
  ...
  const leftGuilds = dbGuilds.filter(
    (g1) => !guilds.some((g2) => g1.guild_id === g2.id)
  );
  for (const guild of leftGuilds) {
    ...
    client.logger.info(`TITAN GAME BOT has left ${guild.guild_name}`);
  }

  client.logger.info("TITAN GAME BOT is now online");
  client.logger.info(
    `TITAN GAME BOT is running on ${client.guilds.cache.size} server(s)`
  );
};
