...
module.exports = (client, oldMessage, newMessage) => {
  ...
  if (oldMessage.content != newMessage.content) {
    ...
    const starboardChannel =
      newMessage.guild.channels.cache.get(starboardChannelId);
    ...
    const messageEditLog =
      newMessage.guild.channels.cache.get(messageEditLogId);
    if (
      messageEditLog &&
  ...
  if (oldMessage.embeds.length > newMessage.embeds.length) {
    ...
    const messageDeleteLog =
      newMessage.guild.channels.cache.get(messageDeleteLogId);
    if (
      messageDeleteLog &&
